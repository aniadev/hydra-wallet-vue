...
import { bytesToHex } from '../utils/parser'
import * as BaseEncoding from '@scure/base'
import { deserializeTx, deserializeTxHash } from '../utils/deserializer'
import { Serialization } from '@cardano-sdk/core'
import * as Crypto from '@cardano-sdk/crypto'
import { resolveTxHash } from '../utils/resolver'
import type { DataSignature } from '../types'
...
export class WalletStaticMethods {
  ...
  static getAddresses(
    ...
    const baseAddress = buildBaseAddress(
      networkId,
      Hash28ByteBase16.fromEd25519KeyHashHex(paymentKey.toPublic().hash().hex()),
      Hash28ByteBase16.fromEd25519KeyHashHex(stakingKey.toPublic().hash().hex())
    ).toAddress()
  ...
  static addWitnessSets(txHex: string, witnesses: VkeyWitness[]): string {
    ...
    const witnessSetVkeys = witnessSet.vkeys()
    const witnessSetVkeysValues: Serialization.VkeyWitness[] = witnessSetVkeys
...
export class EmbeddedWallet extends WalletStaticMethods {
  ...
  getAccount(accountIndex = 0, keyIndex = 0): Account {
    ...
    const { paymentKey, stakeKey, dRepKey } = buildKeys(this._walletSecret, accountIndex, keyIndex)
