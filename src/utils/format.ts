...
export function initListToken(
  listToken: IAssetToken[],
  listAllToken: IAssetToken[],
): void {
...
export function truncateToDecimals(num: number, dec: string): string {
  ...
  return Number(
    (Math.trunc(num * calcDec) / calcDec).toFixed(ROUNDING_CURRENCY[dec]),
  ).toLocaleString('en-US', {
    minimumFractionDigits: ROUNDING_CURRENCY[dec],
  })
}

export function recursiveToCamel<T extends Record<string, any>>(
  item: Record<string, any>,
): T {
  if (Array.isArray(item)) {
    return item.map((el: Record<string, any>) =>
      recursiveToCamel(el),
