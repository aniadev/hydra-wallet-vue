...
export async function computePPViewHashBrowser(protocolParams: typeof defaultPP): Promise<string> {
  const ppView = {
    coinsPerUtxoByte: protocolParams.utxoCostPerByte,
    prices: {
      memory: protocolParams.executionUnitPrices.priceMemory,
      steps: protocolParams.executionUnitPrices.priceSteps
    },
  ...
  const cborBytes = cbor.encodeCanonical(ppView)

  // SHA-256 bằng Web Crypto API
  const hashBuffer = await window.crypto.subtle.digest('SHA-256', cborBytes)

  // Convert ArrayBuffer → hex string
  const hashArray = Array.from(new Uint8Array(hashBuffer))
  const hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join('')

  // dùng TransactionHash thay cho SafeHash
  const hash = CardanoWasm.TransactionHash.from_bytes(new Uint8Array(hashBuffer))
  return hash.to_hex()
}
