...
<template>
  <div class="flex h-full w-full flex-col justify-between bg-[#fff]">
    ...
    <div
      class="flex-grow-1 flex flex-col items-center justify-center overflow-y-hidden p-4"
      v-if="currentWallet"
    >
      <template v-if="fastTransferStatus === 'AVAILABLE'">
        <SelectionScreen
          v-if="hydraState !== HydraState.OPEN"
          @prepare-success="onPreparingSuccess"
        />
        <TransferScreen
          v-else-if="
            hydraState === HydraState.OPEN &&
            currentWallet &&
            sessionData.mnemonic
          "
