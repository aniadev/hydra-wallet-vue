  ...
  <div class="flex h-full w-full flex-col justify-between p-4 shadow-xl">
    <div class="flex justify-end" v-if="inTeleApp && !isInitializingTelegram">
      <a-button
        ...
        class=""
        size="large"
        @click="$router.push({ name: 'AuthImport' })"
        >Login</a-button
      >
    </div>
    <div class="flex flex-col items-center">
      ...
      <p class="text-body-1 font-700 text-center">
        Next generation Telegram wallet. <br />Secure, Fast and over the Cardano
        Blockchain
      </p>
      <p class="text-body-1 font-400 text-center" v-if="inTeleApp && ready">
        Welcome back,
        <span class="text-primary"
          >@{{ teleApp.initDataUnsafe.user?.username }}</span
        >
      </p>
      <p
        class="text-body-1 font-400 text-center"
        v-if="inTeleApp && isInitializingTelegram"
      >
