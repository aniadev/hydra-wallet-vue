...
export const useWalletCore = () => {
  ...
  function getSigningKey(rootKey: CardanoWasm.Bip32PrivateKey): PaymentSigningKey {
    const privateKey = getPrivateKey(rootKey)
    const extendedKey = privateKey.to_hex().slice(4, 68)
    return new PaymentSigningKey(extendedKey)
  }

  function getVerificationKey(skey: PaymentSigningKey): PaymentVerificationKey {
    return new PaymentVerificationKey(skey)
  }

  function getPrivateKey(rootKey: CardanoWasm.Bip32PrivateKey): CardanoWasm.PrivateKey {
  ...
  const walletCore = {
    ...
    // functions from support skey, vkey
    getSigningKey,
    getVerificationKey,
    getPrivateKey,
    CardanoWasm
